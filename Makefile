...
.PHONY: install
## `install`: Fetch dependencies needed to run `redhat-openai-bot`
install:
...
.PHONY: run
## :
## `run`: Run `redhat-openai-bot` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `redhat-openai-bot` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `redhat-openai-bot`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `redhat-openai-bot`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
