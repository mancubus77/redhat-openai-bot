...
a docker containers for *redhat-openai-bot*!
...
```bash
make docker-gen IMAGE=redhat-openai-bot VERSION=latest
```

By default, the values for these environment variables are; <br>

```bash
IMAGE := redhat-openai-bot
VERSION := latest
```
...
```bash
make clean-docker IMAGE=redhat-openai-bot VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *redhat-openai-bot* uses
...
```bash
make docker-debug IMAGE=redhat-openai-bot-debug VERSION=latest
```
...
```bash
docker run redhat-openai-bot-debug
```

You can run a debug image, and directly `shell` into it with

```bash
docker run --rm -it redhat-openai-bot-debug bash
```
...
```bash
make clean-docker IMAGE=redhat-openai-bot-debug VERSION=latest
```
